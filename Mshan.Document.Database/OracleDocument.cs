...
namespace Mshan.Document.Database
{
   ...
   */
        ...
        public static DataTable GetTablesByUser(string user)
        {
            ...
            return DbHelper.Fill(string.Format("select a.*,b.comments from dba_tables a left join dba_tab_comments b  on a.table_name=b.table_name where lower(a.owner)='{0}'", user.ToLower()));
        }
