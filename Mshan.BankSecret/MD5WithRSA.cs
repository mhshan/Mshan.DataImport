...
namespace Mshan.BankSecret
{
    public class MD5WithRSA
    {
        ...
        public static bool VerifyData(string publicKey, string password, string strdata,string sign, string encoding = "ISO-8859-1")
        {
            ...
            {
                objx5092 = new X509Certificate2(encoder.GetBytes(publicKey), password);
            }
            RSACryptoServiceProvider rsa = objx5092.PrivateKey as RSACryptoServiceProvider;
            byte[] data = Encoding.GetEncoding(encoding).GetBytes(strdata);
            byte[] hashvalue = rsa.SignData(data, "MD5");//为证书采用MD5withRSA 签名
            return rsa.VerifyData(data, "MD5", encoder.GetBytes(sign));
        }
        /// <summary>
        /// 将签名结果转化为16进制字符串
        /// </summary>
